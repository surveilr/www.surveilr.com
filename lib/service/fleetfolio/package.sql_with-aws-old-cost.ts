     ...
     SELECT
      ...
      SELECT
       ...
       -- Dashboard count
        ...
        select
            "Cost Report (FOCUS Standard)"  as title,
            "chart-bar" as icon,
            'green' as color,
            ${this.absoluteURL("/fleetfolio/cost_report_focus_standard.sql")
      } as link;
     `;
  }

  @spn.shell({ breadcrumbsFromNavStmts: "no" })
  "fleetfolio/cost_report_focus_standard.sql"() {
    ...
    return this.SQL`
      ...
      SELECT 'Cost Report (FOCUS Standard)' AS title, ${this.absoluteURL("/fleetfolio/cost_report_focus_standard.sql")
      } AS link;

      SELECT 'title' AS component, 'AWS Cost Report (FOCUS Standard)' AS contents;
      ...
      SELECT 'table' AS component,
        ...
        'Region' as 'region_id',
        'Contracted Cost (Billing Currency)' as 'contracted_cost',
        'Consumed Quantity' as 'consumed_quantity',
        'Billed Cost (Billing Currency)' as 'billed_cost',
        'Provider Name' as 'provider_name',
        'List Cost (Billing Currency)' as 'list_cost';
      ...
      SELECT 
        ...
        region_id AS "Region",
        ROUND(contracted_cost, 2) || ' (' || billing_currency || ')' AS "Contracted Cost (Billing Currency)",
        ROUND(consumed_quantity, 2) AS "Consumed Quantity",
        ROUND(billed_cost, 2) || ' (' || billing_currency || ')' AS "Billed Cost (Billing Currency)",
        provider_name AS "Provider Name",
        ROUND(list_cost, 2) || ' (' || billing_currency || ')' AS "List Cost (Billing Currency)"
      FROM ${viewName}
