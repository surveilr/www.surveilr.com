...
SELECT
    ...
    (SELECT study_id FROM uniform_resource_study LIMIT 1) || '-' || DeidentID AS participant_id,  -- Concatenates study_id and DeidentID to form participant_id
    ...
    CASE
        ...
        ELSE 'F'
    END AS gender,  -- Maps Gender to 'M' or 'F'
    Race || ', ' || Ethnicity AS race_ethnicity,  -- Concatenates Race and Ethnicity for race_ethnicity
    CAST(Age AS INTEGER) AS age,  -- Converts Age to REAL type
    BMI AS bmi,  -- Maps BMI
    Hba1c_POC AS baseline_hba1c,  -- Maps HbA1C to baseline_hba1c
    'Type 1 Diabetes' AS diabetes_type,  -- Sets diabetes_type
    '' AS study_arm  -- Placeholder for study arm
FROM
    uniform_resource_cgmndscreening
WHERE    
    Age > 0 AND PtEligible = 'Yes';
...
SELECT
    ...
    CAST(Value as REAL) as CGM_Value     
FROM
