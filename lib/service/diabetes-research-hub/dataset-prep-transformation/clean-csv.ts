...
async function cleanCsv(filePath: string, outputFolder: string, baseDate: string = '2012-01-01'): Promise<void> {
    try {
        ...
        const finalRows = cleanedRows.map((row, index) => {
            ...
            return [row[0].replace(baseDate, newDate.toISOString().split('T')[0]), ...row.slice(1)]; // Replace the date
        });
...
async function processFilesInFolder(inputFolder: string, baseDate: string, outputSubfolderName: string = 'detrended-fluctation-analysis-csv-dataset') {
