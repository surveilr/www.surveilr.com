...
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
  ...
  "image": "https://www.surveilr.com/assets/brand/content-assembler.png",
  "layout": "fluid",
  "fixed_top_menu": true,
  "link": "index.sql",
  "menu_item": [
...
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
      ...
      'SELECT ''shell'' AS component,
       ...
       ''https://www.surveilr.com/assets/brand/content-assembler.png'' AS image,
       ''fluid'' AS layout,
...
SET current_page = ($offset / $limit) + 1;
 SELECT ''table'' AS component,
  ...
  OFFSET $offset;
  SELECT ''text'' AS component,
    (SELECT CASE WHEN $current_page > 1 THEN ''[Previous](?limit='' || $limit || ''&offset='' || ($offset - $limit) ||  ''&periodical_uniform_resource_id='' || replace($periodical_uniform_resource_id, '' '', ''%20'') ||   '')'' ELSE '''' END) || '' '' ||
    ''(Page '' || $current_page || '' of '' || $total_pages || ") " ||
    (SELECT CASE WHEN $current_page < $total_pages THEN ''[Next](?limit='' || $limit || ''&offset='' || ($offset + $limit) ||   ''&periodical_uniform_resource_id='' || replace($periodical_uniform_resource_id, '' '', ''%20'') ||  '')'' ELSE '''' END)
    AS contents_md 
;
            ',
      CURRENT_TIMESTAMP)
