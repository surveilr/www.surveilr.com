...
SET current_page = ($offset / $limit) + 1;
 SELECT ''table'' AS component,
  ...
  SELECT ''text'' AS component,
    ...
    || '' ''
    || ''(Page '' || $current_page || '' of '' || $total_pages || ") "
    || (SELECT CASE WHEN CAST($current_page AS INTEGER) < CAST($total_pages AS INTEGER) THEN ''[Next](?limit='' || $limit || ''&offset='' || ($offset + $limit) || ''&periodical_uniform_resource_id='' || replace($periodical_uniform_resource_id, '' '', ''%20'') || '')'' ELSE ''xxxxxxx'' END)
    AS contents_md
;
        ;
            ',
