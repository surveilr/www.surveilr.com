...
INSERT INTO sqlpage_aide_navigation (namespace, parent_path, sibling_order, path, url, caption, abbreviated_caption, title, description,elaboration)
VALUES
    ('prime', 'index.sql', 1, 'sq/index.sql', 'sq/index.sql', 'Site Quality', NULL, NULL, 'The Site Quality Explorer (SQE) is a surveilr pattern designed for conducting detailed and ongoing audits of website content quality. It helps analyze how web pages perform across several key SEO factors, providing actionable insights for improving search engine rankings. SQE integrates with surveilr, leveraging its robust ingestion framework to capture and store web content across multiple sources. It uses SQLite and Full Text Search(FTS) to perform efficient queries and generate important SEO metrics', NULL),
    ('prime', 'sq/index.sql', 4, 'sq/missing-meta-data.sql', 'sq/missing-meta-data.sql', 'Mising Meta Data', NULL, NULL, 'Mising Meta Data', NULL)
ON CONFLICT (namespace, parent_path, path)
DO UPDATE SET title = EXCLUDED.title, abbreviated_caption = EXCLUDED.abbreviated_caption, description = EXCLUDED.description, url = EXCLUDED.url, sibling_order = EXCLUDED.sibling_order;
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
      ...
      '{
  "component": "shell",
  "title": "Site Quality Explorer",
  "icon": "",
  "favicon": "https://www.surveilr.com/assets/brand/site-quality-favicon.ico",
  "image": "https://www.surveilr.com/assets/brand/site-quality-icon.png",
  "layout": "fluid",
  "fixed_top_menu": true,
  "link": "index.sql",
  "menu_item": [
...
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
      ...
      'SELECT ''shell'' AS component,
       ''Site Quality Explorer'' AS title,
       NULL AS icon,
       ''https://www.surveilr.com/assets/brand/site-quality-favicon.ico'' AS favicon,
       ''https://www.surveilr.com/assets/brand/site-quality-icon.png'' AS image,
       ''fluid'' AS layout,
...
WITH RECURSIVE breadcrumbs AS (
    SELECT
        ...
        namespace
    FROM sqlpage_aide_navigation
    WHERE namespace = ''prime'' AND path=''sq/index.sql''
    UNION ALL
    SELECT
        COALESCE(nav.abbreviated_caption, nav.caption) AS title,
        COALESCE(nav.url, nav.path) AS link,
...
SELECT
  hostname AS title,
  sqlpage.environment_variable(''SQLPAGE_SITE_PREFIX'') || ''/sq/missing-meta-information.sql?hostname='' || hostname AS link,
  ''world'' AS icon,
  ''blue'' AS color
FROM
