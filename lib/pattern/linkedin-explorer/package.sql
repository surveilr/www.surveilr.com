...
DELETE FROM sqlpage_aide_navigation WHERE parent_path='lie'||'/index.sql';
INSERT INTO sqlpage_aide_navigation (namespace, parent_path, sibling_order, path, url, caption, abbreviated_caption, title, description,elaboration)
VALUES
    ('prime', 'index.sql', 1, 'lie/index.sql', 'lie/index.sql', 'LinkedIn Explorer', NULL, NULL, 'LinkedIn Explorer, will enable LinkedIn data integration into Surveilr. It includes manual data export, automated CSV ingestion, SQL Views for querying, and a Web UI for data analysis.', NULL),
    ('prime', 'lie/index.sql', 1, 'lie/profile.sql', 'lie/profile.sql', 'Profile', NULL, NULL, 'The Source List page provides a streamlined view of all collected content sources. This page displays only the origins of the content, such as sender information for email sources, making it easy to see where each piece of content came from. Use this list to quickly review and identify the various sources contributing to the curated content collection.', NULL)
ON CONFLICT (namespace, parent_path, path)
DO UPDATE SET title = EXCLUDED.title, abbreviated_caption = EXCLUDED.abbreviated_caption, description = EXCLUDED.description, url = EXCLUDED.url, sibling_order = EXCLUDED.sibling_order;
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
  ...
  "icon": "",
  "favicon": "https://www.surveilr.com/assets/brand/linkedin-explorer-favicon.ico",
  "image": "https://www.surveilr.com/assets/brand/linkedin-explorer-icon.png",
  "layout": "fluid",
  "fixed_top_menu": true,
  "link": "index.sql",
  "menu_item": [
...
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
      ...
      'SELECT ''shell'' AS component,
       ...
       NULL AS icon,
       ''https://www.surveilr.com/assets/brand/linkedin-explorer-favicon.ico'' AS favicon,
       ''https://www.surveilr.com/assets/brand/linkedin-explorer-icon.png'' AS image,
       ''fluid'' AS layout,
...
WITH RECURSIVE breadcrumbs AS (
    SELECT
        ...
        namespace
    FROM sqlpage_aide_navigation
    WHERE namespace = ''prime'' AND path=''lie/index.sql''
    UNION ALL
    SELECT
        COALESCE(nav.abbreviated_caption, nav.caption) AS title,
        COALESCE(nav.url, nav.path) AS link,
...
FROM breadcrumbs ORDER BY level DESC;
  ...
  SELECT caption as title, sqlpage.environment_variable(''SQLPAGE_SITE_PREFIX'') || ''/'' || COALESCE(url, path) as link, description
    FROM sqlpage_aide_navigation
   WHERE namespace = ''prime'' AND parent_path = ''lie''||''/index.sql''
   ORDER BY sibling_order;
            ',
      CURRENT_TIMESTAMP)
  ON CONFLICT(path) DO UPDATE SET contents = EXCLUDED.contents, last_modified = CURRENT_TIMESTAMP;
...
--- Dsply Page Title
 ...
 -- sets up $limit, $offset, and other variables (use pagination.debugVars() to see values in web-ui)
    SET total_rows = (SELECT COUNT(*) FROM linkedin_employment_timeline );
SET limit = COALESCE($limit, 50);
SET offset = COALESCE($offset, 0);
SET total_pages = ($total_rows + $limit - 1) / $limit;
SET current_page = ($offset / $limit) + 1;
...
''Ingest and organize employment data from LinkedIn positions to visualize career timelines and analyze professional history seamlessly.'' as contents;
 ...
 -- Display uniform_resource table with pagination
    SELECT ''table'' AS component,
     ...
     SELECT
        * 
        FROM linkedin_employment_timeline
        LIMIT $limit
      OFFSET $offset;
      SELECT ''text'' AS component,
    (SELECT CASE WHEN $current_page > 1 THEN ''[Previous](?limit='' || $limit || ''&offset='' || ($offset - $limit) ||     '')'' ELSE '''' END) || '' '' ||
