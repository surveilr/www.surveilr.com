...
VALUES
    ...
    ('prime', '/opsfolio', 6, 'opsfolio/info/control/control_detail.sql', 'opsfolio/info/control/control_detail.sql', 'Control Details', NULL, NULL, NULL, NULL)
ON CONFLICT (namespace, parent_path, path)
DO UPDATE SET title = EXCLUDED.title, abbreviated_caption = EXCLUDED.abbreviated_caption, description = EXCLUDED.description, url = EXCLUDED.url, sibling_order = EXCLUDED.sibling_order;
...
FROM breadcrumbs ORDER BY level DESC;
              -- not including page title from sqlpage_aide_navigation
                 ...
                 SELECT ''title'' AS component, (SELECT COALESCE(title, caption)
    FROM sqlpage_aide_navigation
   WHERE namespace = ''prime'' AND path = ''opsfolio/info/control/control_dashboard.sql/index.sql'') as contents;
    ;
 SELECT
 ''card''             as component
SELECT DISTINCT
...
FROM breadcrumbs ORDER BY level DESC;
              -- not including page title from sqlpage_aide_navigation
                 ...
                 SELECT ''title'' AS component, (SELECT COALESCE(title, caption)
    FROM sqlpage_aide_navigation
   WHERE namespace = ''prime'' AND path = ''opsfolio/info/control/control.sql/index.sql'') as contents;
    ;
 SELECT
 ''card''             as component
SELECT DISTINCT
