...
import { constants as fsConsts, readdirSync, statSync } from "fs";
  ...
  readdirSync(activePath).forEach((file) => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      results = results.concat(
        walkDirectory(basePath, resolvedPath, proxyConf),
      );
    } else {
      for (const pc of proxyConf) {
        if (pc.acceptFn(activePath, stat)) {
          results.push({
            relativePath: resolvedPath.replace(`${basePath}/`, ""),
            resolvedPath,
            ...pc,
            stat,
          });
...
export async function getStaticPaths() {
  ...
  const acceptedPaths = walkDirectory(basePath, basePath, [
    {
      acceptFn: (path) => path.startsWith("lib/assurance"),
      endpointFn: typicalEndpointsFn,
    },
    {
      acceptFn: (path) =>
        path.startsWith("lib/pattern") &&
        path.indexOf("pattern/fhir-explorer/ingest") == -1,
      endpointFn: typicalEndpointsFn,
    },
    {
  ...
  return acceptedPaths.flatMap((path) =>
    path.endpointFn(path.relativePath, path.resolvedPath, path.stat)
  );
}
