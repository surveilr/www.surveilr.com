-- code provenance: `TypicalSqlPageNotebook.commonDDL` (file:///home/ditty/workspaces/github.com/dittybijil/www.surveilr.com-fork/lib/std/notebook/sqlpage.ts)
...
DROP VIEW IF EXISTS test-cases;
...
GROUP BY 
    uri;
-- code provenance: `ConsoleSqlPages.infoSchemaDDL` (file:///home/ditty/workspaces/github.com/dittybijil/www.surveilr.com-fork/lib/std/web-ui-content/console.ts)
...
VALUES
    ...
    ('prime', '/console', 2, '/console/migrations', '/console/migrations/', 'RSSD Lifecycle (migrations)', 'Migrations', NULL, 'Explore RSSD Migrations to determine what was executed and not', NULL)
...
INSERT OR REPLACE INTO code_notebook_cell (notebook_kernel_id, code_notebook_cell_id, notebook_name, cell_name, interpretable_code, interpretable_code_hash, description) VALUES (
  ...
  '      -- code provenance: `ConsoleSqlPages.infoSchemaContentDML` (file:///home/ditty/workspaces/github.com/dittybijil/www.surveilr.com-fork/lib/std/web-ui-content/console.ts)
...
INSERT INTO sqlpage_aide_navigation (namespace, parent_path, sibling_order, path, url, caption, abbreviated_caption, title, description,elaboration)
VALUES
    ('prime', '/', 1, '/qltyfolio', '/qltyfolio/', 'Test Management Summary', NULL, NULL, 'Test management system', NULL),
    ('prime', '/qltyfolio', 1, '/qltyfolio/test-management.sql', '/qltyfolio/test-management.sql', 'Qualityfolio', NULL, NULL, NULL, NULL)
ON CONFLICT (namespace, parent_path, path)
DO UPDATE SET title = EXCLUDED.title, abbreviated_caption = EXCLUDED.abbreviated_caption, description = EXCLUDED.description, url = EXCLUDED.url, sibling_order = EXCLUDED.sibling_order;
...
-- code provenance: `ConsoleSqlPages.console/content/action/regenerate-auto.sql` (file:///home/ditty/workspaces/github.com/dittybijil/www.surveilr.com-fork/lib/std/web-ui-content/console.ts)
...
WITH RECURSIVE breadcrumbs AS (
    SELECT
        ...
        namespace
    FROM sqlpage_aide_navigation
    WHERE namespace = ''prime'' AND path = ''/console/migrations''
    UNION ALL
    SELECT
        COALESCE(nav.abbreviated_caption, nav.caption) AS title,
        COALESCE(nav.url, nav.path) AS link,
...
SELECT title, link FROM breadcrumbs ORDER BY level DESC;
              -- not including page title from sqlpage_aide_navigation
                ...
                SELECT ''title'' AS component, (SELECT COALESCE(title, caption)
    FROM sqlpage_aide_navigation
   WHERE namespace = ''prime'' AND path = ''/qltyfolio/test-management.sql'') as contents;
    ;
...
INSERT INTO sqlpage_files (path, contents, last_modified) VALUES (
      ...
      '              SELECT ''dynamic'' AS component, sqlpage.run_sql(''shell/shell.sql'') AS properties;
              ...
              SELECT ''title'' AS component, (SELECT COALESCE(title, caption)
    FROM sqlpage_aide_navigation
   WHERE namespace = ''prime'' AND path = ''/qltyfolio/detail.sql'') as contents;
    ;
...
from {$name} where folder_name=$name and file_extension=''md'';
            ',
      CURRENT_TIMESTAMP)
  ON CONFLICT(path) DO UPDATE SET contents = EXCLUDED.contents, last_modified = CURRENT_TIMESTAMP;
